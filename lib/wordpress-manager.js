  ...
  async installTheme(projectName, source, activate = false) {
    ...
    try {
      if (source.startsWith('http')) {
        ...
        const themePath = path.join('/tmp', `${themeName}.zip`);
        ...
        // Extract theme
        await execa('unzip', ['-o', themePath, '-d', themesDir]);
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Theme ${themeName} installed successfully${activate ? ' and activated' : ''}`,
          },
        ],
      };
    } catch (error) {

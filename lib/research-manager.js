...
export class ResearchManager {
  ...
  async researchTopic(projectName, query) {
    ...
    try {
      // Use Jina AI Reader API for research
      ...
      await fs.writeJson(path.join(researchDir, filename), {
        query,
        timestamp,
        results: data,
      }, { spaces: 2 });
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Research completed for "${query}"\n` +
                  `Results saved to: research/${filename}\n` +
